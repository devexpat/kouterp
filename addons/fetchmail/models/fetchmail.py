...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/Kouterp_mailgate.py')
    ...
    def onchange_server_type(self):
        ...
        self.configuration = """
            ...
            Kouterp_mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
            Example configuration for the postfix mta running locally:
            /etc/postfix/virtual_aliases:
            @youdomain Kouterp_mailgate@localhost
            /etc/aliases:
            Kouterp_mailgate: "|/path/to/Kouterp-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
