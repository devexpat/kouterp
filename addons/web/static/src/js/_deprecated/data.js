 ...
 * consumed by Kouterp's RPC APIs.
 *
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<Kouterp.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of Kouterp records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the Kouterp model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // Kouterp ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // Kouterp ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, Kouterp domain
     ...
     * an SQL-like form usable by Kouterp's ORM.
     *
...
var Model = Class.extend({
    /**
    new Kouterp.web.Model(name[, context[, domain]])

    @constructs instance.web.Model
    @extends instance.web.Class

    @param {String} name name of the Kouterp model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound Kouterp model.
     *
